...
davinci-resolve/
  ...
  knowledge/                     Resolve 概念、页面和工作流知识层
  knowledge/scripting/           API 能力矩阵和低风险脚本示例
  references/                    可复用策略、schema、模板和示例
  schemas/                       机器可读 schema
  sources/                       知识来源注册表和版本矩阵模板
  evals/                         安全与幻觉检查测试任务
  evals/cases/                   机器可读行为 eval cases
  adapters/generic-agent/        平台无关运行时能力合同
  scripts/                       校验工具
```
...
- `knowledge/scripting/confirmed_examples/`
...
详细开发规划、版本路线、目录演进、Action Card 升级规范、API 能力矩阵设计和 eval 计划见：
